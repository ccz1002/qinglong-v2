...
exports.default = async () => {
    ...
    if (!(authConfig === null || authConfig === void 0 ? void 0 : authConfig.info) || (0, util_1.isDemoEnv)()) {
        let authInfo = {
            username: 'admin',
            password: 'admin',
        };
    ...
    // 初始化青龙面板默认菜单 (Art Design Pro 后端模式)
    try {
        const { Menu } = await Promise.resolve().then(() => __importStar(require('../data/menu')));
        const menuCount = await Menu.count();
        if (menuCount === 0) {
            const defaultMenus = [
                { name: 'Crontab', path: '/crontab', component: '/qinglong/crontab/index', title: '定时任务', icon: 'ri:timer-line', sort: 1 },
                { name: 'Subscription', path: '/subscription', component: '/qinglong/subscription/index', title: '订阅管理', icon: 'ri:git-repository-line', sort: 2 },
                { name: 'Env', path: '/env', component: '/qinglong/env/index', title: '环境变量', icon: 'ri:settings-3-line', sort: 3 },
                { name: 'Config', path: '/config', component: '/qinglong/config/index', title: '配置文件', icon: 'ri:file-list-3-line', sort: 4 },
                { name: 'Script', path: '/script', component: '/qinglong/script/index', title: '脚本管理', icon: 'ri:code-box-line', sort: 5 },
                { name: 'Dependence', path: '/dependence', component: '/qinglong/dependence/index', title: '依赖管理', icon: 'ri:archive-line', sort: 6 },
                { name: 'Log', path: '/log', component: '/qinglong/log/index', title: '日志管理', icon: 'ri:file-text-line', sort: 7 },
                { name: 'Diff', path: '/diff', component: '/qinglong/diff/index', title: '对比工具', icon: 'ri:file-diff-line', sort: 8 },
                { name: 'Setting', path: '/setting', component: '/qinglong/setting/index', title: '系统设置', icon: 'ri:settings-2-line', sort: 9 },
                { name: 'System', path: '/system', component: '/index/index', title: '系统管理', icon: 'ri:settings-line', sort: 10 },
            ];
            for (const menu of defaultMenus) {
                await Menu.create(menu);
            }
            // 创建系统管理子菜单
            const sysMenu = await Menu.findOne({ where: { name: 'System' } });
            if (sysMenu) {
                await Menu.create({ name: 'MenuManage', path: 'menu', component: '/system/menu/index', title: '菜单管理', icon: 'ri:menu-line', parentId: sysMenu.id, sort: 1 });
                await Menu.create({ name: 'UserManage', path: 'user', component: '/system/user/index', title: '用户管理', icon: 'ri:user-settings-line', parentId: sysMenu.id, sort: 2 });
                await Menu.create({ name: 'RoleManage', path: 'role', component: '/system/role/index', title: '角色管理', icon: 'ri:shield-user-line', parentId: sysMenu.id, sort: 3 });
            }
            for (const menu of defaultMenus) {
                await Menu.create(menu);
            }
            logger_1.default.info('✌️ Default menus seeded');
        }
    }
    catch (error) {
        logger_1.default.warn(`Menu seed skipped: ${error.message}`);
    }
