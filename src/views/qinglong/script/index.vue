<template>
  <div class="script-page">
    <div class="script-toolbar">
      <span class="current-file">{{ currentFile || '请选择脚本文件' }}</span>
      <span v-if="currentNode?.size" class="file-size">{{ formatSize(currentNode.size) }}</span>
      <div class="toolbar-actions">
        <template v-if="isEditing">
          <ElButton type="primary" size="small" @click="saveFile">保存</ElButton>
          <ElButton size="small" @click="cancelEdit">退出编辑</ElButton>
        </template>
        <template v-else>
          <ElButton size="small" @click="addFile">创建</ElButton>
          <ElButton size="small" :disabled="!currentNode" @click="editFile">编辑</ElButton>
          <ElButton size="small" :disabled="!currentNode" @click="renameFile">重命名</ElButton>
          <ElButton size="small" :disabled="!currentNode||currentNode.type==='directory'" @click="downloadFile">下载</ElButton>
          <ElButton size="small" :disabled="!currentNode" type="danger" @click="deleteFile">删除</ElButton>
        </template>
      </div>
    </div>
    ...
    <div class="script-body">
      <div class="script-tree">
        <ElInput v-model="searchText" placeholder="搜索脚本名" size="small" clearable class="tree-search" />
        <ElTree
          :data="filteredTree"
      ...
      <div class="script-editor">
        <div v-if="!currentFile" class="editor-empty">请选择脚本文件</div>
        <div v-else-if="!canPreview" class="editor-empty">
          <p>不支持预览此文件类型</p>
          <ElButton size="small" @click="canPreview=true;loadContent(currentNode)">强制打开</ElButton>
        </div>
        <VueMonacoEditor v-else v-model:value="content" :language="editorLang" :theme="editorTheme" :options="editorOpts" height="100%" @mount="onEditorMount" />
      </div>
    </div>
    ...
    <ElDialog v-model="createVisible" title="创建" width="450px">
      <ElForm label-width="70px">
        ...
        <ElFormItem v-if="createType==='blank'" label="文件名">
          <ElInput v-model="createName" placeholder="example.js" />
        </ElFormItem>
        <ElFormItem v-if="createType==='upload'" label="选择文件">
          <input type="file" ref="fileInputRef" @change="onFilePicked" />
        </ElFormItem>
        <ElFormItem v-if="createType==='directory'" label="文件夹名">
          <ElInput v-model="createDir" placeholder="mydir" />
        </ElFormItem>
      ...
      <template #footer>
        ...
        <ElButton type="primary" @click="doCreate" :loading="opLoading">确定</ElButton>
      </template>
    </ElDialog>
    ...
    <ElDialog v-model="renameVisible" title="重命名" width="400px">
      ...
      <template #footer>
        ...
        <ElButton type="primary" @click="doRename" :loading="opLoading">确定</ElButton>
      </template>
    </ElDialog>
  </div>
</template>
...
const loadTree = async () => {
  try { treeData.value = await request.get<any>({ url: '/api/scripts' }) || [] }
  catch {}
}
...
const doCreate = async () => {
  ...
  if (createType.value === 'upload') {
    ...
    const fn = createType.value === 'directory' ? createDir.value : createName.value
    if (!fn) { ElMessage.warning('请输入名称'); return }
  }
  opLoading.value = true
  try {
    if (createType.value === 'upload') {
      // ★ 上传: FileReader 读内容 → JSON 发送
      ...
      // 用 fetch 直接发，能拿到完整响应体
      const store = (await import('@/store/modules/user')).useUserStore()
      const payload: any = { filename: safeName, content, path: createPath.value || './' }
      const resp = await fetch('/api/scripts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: store.accessToken },
      ...
      // 空文件/文件夹: FormData (无 file 字段, multer 不会触发)
      const fd = new FormData()
      const store = (await import('@/store/modules/user')).useUserStore()
      if (createType.value === 'directory') {
        fd.append('directory', createDir.value)
      } else {
        fd.append('filename', createName.value)
        fd.append('content', '')
      }
    ...
    createVisible.value = false; loadTree()
...
const doRename = async () => {
  ...
  opLoading.value = true
  try {
    await request.put({ url: '/api/scripts/rename', data: { filename: currentNode.value.title, path: currentNode.value.parent || '', newFilename: renameNewName.value } })
    ElMessage.success('重命名成功'); renameVisible.value = false; loadTree()
...
const deleteFile = async () => {
  ...
  try {
    await request.del({ url: '/api/scripts', data: { filename: currentNode.value.title, path: currentNode.value.parent || '', type: currentNode.value.type } })
    ElMessage.success('已删除'); currentFile.value = ''; currentNode.value = null; content.value = ''; loadTree()
...
const downloadFile = async () => {
  ...
  try {
    ...
    const resp = await fetch('/api/scripts/download', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: store.accessToken },
      body: JSON.stringify({ filename: currentNode.value.title, path: currentNode.value.parent || '' })
    })
    if (!resp.ok) throw new Error('下载失败')
    const blob = await resp.blob()
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = currentNode.value.title; a.click()
...
const canPreviewFile = (name: string) => {
  const ext = name.split('.').pop()?.toLowerCase() || ''
  return ['js','ts','py','sh','json','txt','md','css','html','xml','yaml','yml','ini','env','toml','sql','go','java','c','cpp','rs','rb','php','swift','kt','vue','less','scss','bat','ps1','conf','cfg','log'].includes(ext)
}
...
<style scoped>
.script-page { height: 100%; display: flex; flex-direction: column; }
.script-toolbar { display: flex; align-items: center; gap: 8px; padding: 8px 12px; border-bottom: 1px solid #eee; background: #fafafa; flex-shrink: 0; }
.current-file { font-weight: 600; flex: 1; }
.file-size { color: #999; font-size: 12px; margin-right: 8px; }
.toolbar-actions { display: flex; gap: 4px; }
.script-body { flex: 1; display: flex; overflow: hidden; }
.script-tree { width: 240px; flex-shrink: 0; border-right: 1px solid #eee; display: flex; flex-direction: column; overflow: hidden; }
.script-tree :deep(.el-tree) { flex: 1; overflow: auto; padding: 4px 0; }
.tree-search { padding: 8px; }
